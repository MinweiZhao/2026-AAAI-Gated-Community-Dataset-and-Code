...
class SimpleRNNClassifier(nn.Module):
    def __init__(self, input_size=490, hidden_size=128, num_layers=1):
    ...
    def forward(self, x):
        x = x.mean(1)  # Take the mean over channels -> [B, H, W]
        output, _ = self.rnn(x)  # [B, T, H], where T=490
        return self.fc(output[:, -1, :])  # take last timestep
...
def train_rnn(exp_name, dataset_root, excel_path, epochs=10, lr=1e-4, batch_size=8):
    model = SimpleRNNClassifier().cuda()
